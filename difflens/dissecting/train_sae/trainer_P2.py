...
from difflens.generation_models.P2.unet import UNetModel
from difflens.generation_models.P2.respace import SpacedDiffusion
...
class P2SaeTrainer:
    def __init__(
        self, cfg: TrainConfig, model: UNetModel, diffusion: SpacedDiffusion,
    ):
        if cfg.hookpoints:
        ...
        input_widths = resolve_widths_P2(model, cfg.hookpoints)
        ...
        self.saes = {
            hook: Sae(input_widths[hook], cfg.sae, device, kernel_size=self.cfg.kernel_size)
        ...
        num_steps = int(1e7 // self.cfg.macro_batch_size)

        self.lr_scheduler = get_linear_schedule_with_warmup(
            self.optimizer, cfg.lr_warmup_ratio*num_steps, num_steps
        )
    ...
    def load_state(self, path: str):
        ...
        if self.cfg.trained_idx == 0:
            # We only load state for the first train
    ...
    def fit(self):
        ...
        if self.cfg.log_to_wandb and rank_zero:
            # Replace Tensorboard
            try:
                log_dir = f"runs/kernel_{self.cfg.kernel_size}/k_{self.cfg.sae.k}/expansion_factor_{self.cfg.sae.expansion_factor}/lr_{self.cfg.lr}"
        ...
        all_timesteps = list(reversed(range(0, 51)))

        def hook(module: nn.Module, _, outputs):
            name = all_timesteps.pop()
            outputs = outputs.contiguous()
            hidden_dict[name] = outputs
        
        for batch in range(num_steps):
            all_timesteps = list(reversed(range(0, 51)))  # Reset for each batch
            hidden_dict.clear()

            # Bookkeeping for dead feature detection
            num_tokens_in_step += self.cfg.macro_batch_size
            ...
            try:
                with torch.no_grad():
                    self.diffusion.ddim_sample_loop(
                        self.model,
                        (self.cfg.macro_batch_size, 3, self.cfg.image_size, self.cfg.image_size),
                    )
            finally:
            ...
            for name, hiddens in hidden_dict.items():
                raw = self.saes[self.local_hookpoints()[0]]  # 'raw' never has a DDP wrapper
                ...
                if not maybe_wrapped:
                    ...
                    maybe_wrapped = (
                        ...
                        if ddp
                        else [self.saes[self.local_hookpoints()[0]]]
                    )
    ...
    def save(self):
        ...
        path = self.cfg.run_name or f"sae-ckpts/kernel_{self.cfg.kernel_size}/k_{self.cfg.sae.k}/expansion_factor_{self.cfg.sae.expansion_factor}/lr_{self.cfg.lr}"
        rank_zero = not dist.is_initialized() or dist.get_rank() == 0
