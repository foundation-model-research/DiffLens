...
def resolve_widths_SD(
    ...
    shapes: dict[str, int] = {}
    h_classification_shapes: dict[str, int] = {}
    ...
    def hook(module, _, output):
        ...
        name = module_to_name[module]
        shapes[name] = output.shape[dim]
        h_classification_shapes[name] = output.shape[-1]*output.shape[-2]*output.shape[-3]

    handles = [
        mod.register_forward_hook(hook) for mod in module_to_name
    ]
    dummy = send_to_device(DUMMY_INPUTS_SD, get_model_device(model.unet))
    try:
        model(**dummy)
    finally:
        for handle in handles:
    ...
    return shapes, h_classification_shapes
