...
class Autoencoder(nn.Module):
    ...
    def __init__(
        ...
        if tied:
            self.decoder: nn.Linear = TiedTranspose(self.encoder)
        else:
...
class TimestepAutoencoder(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.autoencoders = nn.ModuleList([FastAutoencoder(*args, **kwargs) for i in range(49)])
    
    def forward(self, x, t):
        return self.autoencoders[t](x)