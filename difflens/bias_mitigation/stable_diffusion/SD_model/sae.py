...
class Sae(nn.Module):
    ...
    """
    The following code is selecting topk features on c*h*w features
    """
    # def pre_acts(self, x: Tensor) -> Tensor:
    #     # Remove decoder bias as per Anthropic
    #     sae_in = x.to(self.dtype) - self.decoder.bias.view(1, -1, 1, 1)
    ...
    #     # We select the top-k features among h*w features
    #     batch_size = latents.shape[0]
    #     latents_flat = latents.view(batch_size, -1)
    #     return EncoderOutput(*latents_flat.topk(self.cfg.k, sorted=False))
    ...
    #     # Sorry, but we did't implement decoder in triton
    #     assert self.decoder is not None, "Decoder weight was not initialized."

    #     batch_size = top_acts.shape[0]
    #     sparse_latents = torch.zeros((batch_size, self.num_latents*self.h*self.w), 
    #                                device=self.device, dtype=self.dtype)
    #     sparse_latents.scatter_(dim=-1, index=top_indices, src=top_acts)

    #     sparse_latents = sparse_latents.view(batch_size, -1, self.h, self.w)
